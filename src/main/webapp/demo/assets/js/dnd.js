...
$(function () {
    ...
    //开始拖动元素后，给可放入的元素添加监听事件
    var addeve = function (tag, clientFrameWindow) {
        ...
        var htmlBody = $(clientFrameWindow.document).find(tag);
        ...
        $(clientFrameWindow.document).find(tag).on('drop', function (event) {
            event.preventDefault();
            event.stopPropagation();
    ...
    //添加dragstart监听事件
    var dndinit = function (conf) {
        //source是拖动的元素的内容，tag是要放入的目标元素
        var source, tag;
        ...
        $("#dragitemslistcontainer li").on('dragstart', function (event) {
            ...
            source = $(this).attr('data-insert-type');
            for (var i = 0; i < conf.length; i++) {
                if (conf[i].src === source) {
                    tag = conf[i].tag;
                    break;
                }
            }
            ...
            console.log("可放入" + tag + "元素中");
            addeve(tag, clientFrameWindow);
        });
        $("#dragitemslistcontainer li").on('dragend', function () {
            ...
            $(clientFrameWindow.document).find(tag).unbind();
        });
    }

    //请求配置信息
    $.ajax("./assets/js/conf.json", {
        dataType: 'json',
